...
# 2. Select Visa AID
SELECT_AID = [0x00, 0xA4, 0x04, 0x00, 0x0B] + [
    0xA0, 0x00, 0x00, 0x00, 0x77, 0x01, 0x08, 0x01, 0x02, 0x02, 0x02, 0x01
]
response, sw1, sw2 = connection.transmit(SELECT_AID)
print(f"SELECT AID SW1={hex(sw1)} SW2={hex(sw2)}")
if sw1 == 0x61:
    response, sw1, sw2 = get_response(connection, sw2)
print("AID Response:", toHexString(response))
